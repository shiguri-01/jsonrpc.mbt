...
metadata:
    github-path: moonbit-agent-guide
    github-ref: refs/heads/main
    github-repo: https://github.com/moonbitlang/moonbit-agent-guide
    github-tree-sha: 54f51b736b940be78e8043d5cb76b730455f62c7
...
│   └── main
│       ├── main.mbt
│       └── moon.pkg     # executable package with `options(is_main: true)`
...
For more advanced topics like `conditional compilation`, `link configuration`, `warning control`, and `dev_build commands`, see `references/advanced-moonbit-build.md`.
...
1. Add the dependency and pin the native target in `moon.mod`:
   ...
   preferred_target = "native"
   ```
2. In the executable's `moon.pkg`, set `is_main`, restrict to native, and import what you need:
   ```
   import {
     "moonbitlang/async",
     "moonbitlang/async/stdio",
